...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-fd2c47ca";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-fd2c47ca">'+'<span class="tile-face-fd2c47ca tile-front-fd2c47ca"></span>'+'<span class="tile-face-fd2c47ca tile-back-fd2c47ca"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-fd2c47ca")){return;}
var back=tile.querySelector(".tile-back-fd2c47ca");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-fd2c47ca");if(isMine){tile.classList.add("boom-fd2c47ca");}
if(dim){tile.classList.add("dimmed-fd2c47ca");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-fd2c47ca")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-fd2c47ca");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-fd2c47ca")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-fd2c47ca");void panelEl.offsetWidth;panelEl.classList.add("jolt-fd2c47ca");}
endRound();}
